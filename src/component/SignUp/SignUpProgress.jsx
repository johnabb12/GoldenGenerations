...
const SignUpProgress = ({ currentStep, stepValidation = {} }) => {
  ...
  const steps = [
    t('auth.signup.steps.idVerification'),
    t('auth.signup.steps.credentials'),
    t('auth.signup.steps.personalDetails'),
    t('auth.signup.steps.workBackground'),
    t('auth.signup.steps.lifestyle'),
    t('auth.signup.steps.veteransCommunity')
  ];
    ...
    <div className="max-w-4xl mx-auto mt-4 sm:mt-8 px-4">
      ...
      <div className="block sm:hidden mb-6">
        <div className="flex justify-between items-center mb-3">
          <span className="text-sm font-medium text-gray-600">{t('common.step')} {currentStep + 1} {t('common.of')} {steps.length}</span>
